...
func (b *blobService) CreateChunk(path string, id int64) error {
	filePath := filepath.Join(server.GlobalServerDesc.Opt.Volume, path)
...
func (b *blobService) LockFile(path string) error {
	filePath := filepath.Join(server.GlobalServerDesc.Opt.Volume, path)
	_, err := os.Stat(filePath)
	if err == nil {
		lockPath := utils.GetLockPath(filePath)
...
// TODO: Implement COPY and MOVE API
